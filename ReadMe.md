...
...* All these three fields don't need to exist on the server. they only need to exist on a device.
...
#### One user can synchronize his/her list on all his/her devices
...
i.e. another user did the same thing and already synced their changes.
...
...* then it would change the underlying item (shampoos quantity was changed) to the devices changes
...
Amazon Cognito
Simplicity allows amazon to take of all those needs.
...
#### One user can invite a friend to collaborate on a list
#### The friend can accept the invitation
...
The important question to ask first is how exactly will the the request and response will be sent in term of synchronicity.
...
POST /api/server/shoppingList/syncMod/
...
SYNC  1. then this would return the changes made on the server 
ASYNC 2. return a 200 and kick off another process on the server and that will return the changes
